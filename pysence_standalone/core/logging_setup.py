...
IMPORTANT: never pass the Discord token pair (or any api.* field value that
might carry a secret, e.g. an API key baked into a URL) to log calls at
INFO or above. Use redact() below, or just log presence/absence.
"""
...
def apply_options(options: dict):
    ...
    if webhook_urls:
        from pysence_standalone.core.webhook_logger import WebhookLogHandler
        handler = WebhookLogHandler(
            webhook_urls=webhook_urls,
            flush_interval=options.get("script.dev.discord.webhook.interval", 30),
            alias=options.get("script.dev.alias", "pysence-standalone"),
        )
...
def log_bypass(logger: logging.Logger, msg: str, *args, **kwargs):
    ...
    toggles -- for messages that are genuinely important even with quiet
    logging turned on, e.g. 'reconnected after N attempts'."""
